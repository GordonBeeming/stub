...
import { buildShortUrl } from '@/lib/types';
import type { LinkClickRow, LinkRow } from '@/lib/types';
...
function LinkListItem({ link, siteUrl, onToggleDisabled, onDelete }: LinkListItemProps) {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    // Lazy fetch: only fires when the row expands. The setLoading flip is the
    // "external system sync" — it reflects the in-flight HTTP request that's
    // kicked off on the next line. React 19's new lint is tuned for derived
    // state, which this isn't.
  ...
  async function onCopy(e: React.MouseEvent) {
    ...
    try {
      ...
      // Clipboard may be blocked (e.g. no HTTPS in dev). Fall back silently
    ...
    <div>
      ...
      >
        <NameRow
          word={link.id}
          badge={
            badge ? { label: badge.label, variant: badge.variant } : undefined
          }
        >
